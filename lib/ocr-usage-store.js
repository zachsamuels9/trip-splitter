...
async function setActiveMonthlyLimit(limit) {
...
async function readUsage(month) {
  ...
  try {
    ...
    logUsage("persistent read failed; using memory fallback", { month, error: error.message });
    return readMemoryUsage(month);
  }
}
...
async function writeUsage(usage) {
  ...
  try {
    ...
    memoryUsage.set(usage.month, usage);
    logUsage("persistent write failed; using memory fallback", { month: usage.month, error: error.message });
    return usage;
  }
}
...
async function readSetting(key) {
  ...
  try {
    ...
    logUsage("settings read failed; using fallback", { key, error: error.message });
...
async function writeSetting(setting) {
  ...
  try {
    ...
    memorySettings.set(setting.key, setting);
    logUsage("settings write failed; using memory fallback", { key: setting.key, error: error.message });
    return setting;
  }
}

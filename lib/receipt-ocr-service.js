...
const { assertOcrUsageAvailable, recordOcrAttemptResult, recordOcrAttemptStarted, usageMonth } = require("./ocr-usage-store");

const cache = new Map();
const CACHE_LIMIT = 50;

async function processReceiptImage({ imageDataUrl, timeZone = "" }) {
  ...
  const document = await callDocumentAi(imageBase64, mimeType, timeZone);
...
async function callDocumentAi(imageBase64, mimeType, timeZone = "") {
  const month = usageMonth(new Date(), timeZone);
  await assertOcrUsageAvailable(month);
  ...
  await recordOcrAttemptStarted(month);
  try {
    ...
    if (!response.ok) {
      ...
      await recordOcrAttemptResult(false, month);
    ...
    await recordOcrAttemptResult(true, month);
    log("parsed", { entities: document.entities?.length || 0, pages: document.pages?.length || 0 });
    return document;
  } catch (error) {
    if (!error.statusCode) await recordOcrAttemptResult(false, month);
    throw error;
  }
}
...
async function extractStructuredReceipt(document) {
  ...
  const currency = entityCurrency(entities) || detectContextCurrency(document.text || "") || "";
  const receipt = {
  ...
  receipt.lineItems = await normalizeLineItemsWithGpt(receipt.lineItems, {
    ...
    textSample: cleanText((document.text || "").slice(0, 2000)),
  });
...
async function normalizeLineItemsWithGpt(lineItems, context = {}) {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!lineItems.length) return lineItems;
  if (!apiKey) return translateLineItemsWithGoogle(lineItems);
  try {
        ...
        input: [
          {
            role: "system",
            content:
              "Normalize receipt line item names only. Translate Thai and Vietnamese item names to natural English, expand abbreviations, assign a broad category, and keep quantities, prices, totals, and currency unchanged. Do not convert currencies. Return strict JSON array with name, normalizedName, category, quantity, unitPrice, amount.",
          },
          { role: "user", content: JSON.stringify({ context, lineItems }) },
        ],
      }),
    });
    ...
    return translateLineItemsWithGoogle(lineItems);
  }
}

async function translateLineItemsWithGoogle(lineItems) {
  const needsTranslation = lineItems.some((item) => /[\u0E00-\u0E7Fàáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđ]/i.test(item.name || ""));
  if (!needsTranslation) return lineItems;
  try {
    const projectId = requiredEnv("GOOGLE_DOCUMENT_AI_PROJECT_ID");
    const accessToken = await documentAiAccessToken();
    const response = await fetch(`https://translation.googleapis.com/v3/projects/${encodeURIComponent(projectId)}/locations/global:translateText`, {
      ...
      body: JSON.stringify({
        contents: lineItems.map((item) => item.name || "Item"),
        mimeType: "text/plain",
    ...
    return lineItems.map((item, index) =>
      finalizeLineItem({
        ...item,
        normalizedName: cleanText(translations[index]?.translatedText || item.normalizedName || item.name),
      })
    );
  } catch (error) {
    log("google translation skipped", { error: error.message });
    return lineItems;
  }
}
...
function detectContextCurrency(value) {
  const text = String(value || "");
  if (/\$|\bUSD\b/i.test(text)) return "USD";
  if (/฿|\bTHB\b|\bBAHT\b|บาท/i.test(text)) return "THB";
  if (/₫|\bVND\b|\bVNĐ\b|\bDONG\b/i.test(text)) return "VND";
  if (!/[$฿₫]/.test(text) && /(?:\bBANGKOK\b|\bPHUKET\b|\bCHIANG\s*MAI\b|\bTHAILAND\b|[\u0E00-\u0E7F])/i.test(text)) return "THB";
  if (!/[$฿₫]/.test(text) && /(?:\bVIETNAM\b|\bHANOI\b|\bSAIGON\b|\bHO CHI MINH\b|đường|quận|phường|[àáạảãâầấậẩẫăằắặẳẵèéẹẻẽêềếệểễìíịỉĩòóọỏõôồốộổỗơờớợởỡùúụủũưừứựửữỳýỵỷỹđ])/i.test(text)) return "VND";
  return "";
}

...
async function signInAccount({ email, passcode }) {
  ...
  try {
    ...
    const activeRows = rows.filter((row) => byTripId[row.trip_id]);
    return {
...
async function hardDeleteTrip(groupId) {
  ...
  await supabase(`participants?trip_id=eq.${groupId}`, { method: "DELETE" });
  await supabase(`trips?id=eq.${groupId}`, { method: "DELETE" });
}

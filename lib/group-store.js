...
async function listTrips() {
  ...
  const trips = await supabase("trips?select=id,name,status,closed_at,updated_at&order=updated_at.desc");
  const [participants, receipts] = await Promise.all([supabase("participants?select=trip_id"), supabase("receipts?select=trip_id")]);
  return {
    trips: trips.map((trip) => ({
      ...
      receiptCount: receipts.filter((receipt) => receipt.trip_id === trip.id).length,
      updatedAt: trip.updated_at,
    })),
  };
}

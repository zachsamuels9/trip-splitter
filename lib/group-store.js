...
async function signInAccount({ email, passcode }) {
  ...
  try {
    const rows = await supabase(`participants?email=eq.${encodeURIComponent(clean)}&passcode_hash=eq.${passcodeHash(passcode)}&select=id,name,email,trip_id`);
    const tripIds = rows.map((row) => row.trip_id).filter(Boolean);
    const trips = tripIds.length ? await supabase(`trips?id=in.(${tripIds.join(",")})&select=id,name,updated_at`) : [];
    const byTripId = Object.fromEntries(trips.map((trip) => [trip.id, trip]));
    return {
      account: rows[0] ? participantAccount(rows[0], byTripId[rows[0].trip_id]) : null,
      trips: rows.map((row) => participantAccount(row, byTripId[row.trip_id])),
    };
  } catch (error) {
...
async function listTrips() {
  ...
  const trips = await supabase("trips?select=id,name,updated_at&order=updated_at.desc");
  ...
  return {
    trips: trips.map((trip) => ({
      id: trip.id,
      name: trip.name,
      status: receipts.some((receipt) => receipt.trip_id === trip.id && isClosedMarker(receipt)) ? "closed" : "active",
...
async function deleteTrip(groupId) {
  ...
  await supabase(`receipts?trip_id=eq.${group.id}`, { method: "DELETE" });
  await supabase(`participants?trip_id=eq.${group.id}`, { method: "DELETE" });
  await supabase(`trips?id=eq.${group.id}`, { method: "DELETE" });
  return { ok: true };
}

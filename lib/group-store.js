...
async function writeLineItems(receipt) {
  ...
  const assignmentRows = items.flatMap((item) =>
    (item.assignedTo || []).filter(isUuid).map((participantId) => ({
      ...
      share_amount: item.assignedTo?.length ? (item.amount || 0) / item.assignedTo.length : null,
    }))
  );
...
function receiptFromRows(receipt, lineItems, assignments, images, people) {
  const items = lineItems
    ...
    .map((item) => ({
      ...
      assignedTo: assignments.filter((assignment) => assignment.line_item_id === item.id).map((assignment) => assignment.participant_id),
    }));
  const appReceipt = {
    id: receipt.id,
